// # 6. 이상한 문자만들기

// toWeirdCase함수는 문자열 s를 매개변수로 입력받는다.
// 문자열 s에 각 단어의 짝수번째 인덱스 문자는 대문자로, 홀수번째 인덱스 문자는 소문자로
// 바꾼 문자열을 리턴하도록 함수를 완성하라.
// 예를 들어 s가 ‘try hello world’라면 첫 번째 단어는 ‘TrY’, 두 번째 단어는 ‘HeLlO’, 세 번째 단어는 ‘WoRlD’로 바꿔 ‘TrY HeLlO WoRlD’를 리턴한다.

// 주의) 문자열 전체의 짝/홀수 인덱스가 아니라 단어(공백을 기준)별로 짝/홀수 인덱스를 판단한다.

function toWeirdCase(s) {
  
    var str = s.split(' ');
    console.log(str.length);
  
    for (var i = 0; i < str.length; i++) {
      // str[i]에 값을 담아야하기 때문에 str[i]를 1번씩 수행완료될때마다 s2를 초기화해야한다. 
      var s2 = [];
      // str[i]는 space로 구분된 str의 길이만큼 for문 실행
      for (var n = 0; n < str[i].length; n++) {
        // 짝수번째이면 대문자로 홀수번째이면 소문자로 s2에다가 저장한다.
        if (n % 2 == 0) {
          s2 += str[i][n].toUpperCase();
        } else {
          s2 += str[i][n].toLowerCase();
        }
      }
      // s2에 저장된값을 str에다가 저장한다.
      str[i] = s2;
    }
    // join을 통하여 ' '로 구분하여서 
    return str.join(' ');
  
  }
  
  console.log(toWeirdCase('try hello world')); // 'TrY HeLlO WoRlD'`
  console.log(toWeirdCase('hello world')); // ‘HeLlO WoRlD’
  console.log(toWeirdCase('my name is lee')); // ‘My NaMe Is LeE’